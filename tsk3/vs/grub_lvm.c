...
grub_lvm_open (const char *name, grub_disk_t disk)
{
  ...
  if (! lv)
    return (grub_err_t)grub_error ("unknown LVM device %s", name);
...
{
    return grub_get_lv_offset(grub_get_largest_volume());
}

struct grub_lvm_lv *
grub_get_largest_volume()
{
  struct grub_lvm_vg *vg;
  struct grub_lvm_lv *lv = NULL;
  ...
  for (vg = vg_list; vg; vg = vg->next)
    {
      ...
      if (vg->lvs) {
	    ...
	    for (tlv = vg->lvs; tlv; tlv = tlv->next)
	        if (tlv->visible && (lv->size < tlv->size))
                lv = tlv;
        }
    }

    printf("Largest volume: %s size : %ld\n", lv->name, lv->size);
    return lv;
...
grub_get_node_offset (const struct grub_lvm_node *node, grub_uint64_t offset)
{
  ...
  if (node->pv)
    {
	    return node->pv->start+offset;
    }

  if (node->lv)
    return grub_get_lv_offset(node->lv);
  (grub_err_t)grub_error ("unknown node '%s'", node->name);
    return 0;
}
...
grub_get_lv_offset (struct grub_lvm_lv *lv)
{
  ...
  grub_uint64_t ret_offset = 0;
  unsigned int i;
  grub_disk_addr_t sector = 0;

  if (!lv)
    return (grub_err_t)grub_error ("null volume");
  ...
  if (i == lv->segment_count)
    return (grub_err_t)grub_error ("incorrect segment");
  ...
  switch (seg->type)
      ...
      else
	{
      ...
      ///return read_node (node, offset, size, buf);
        ret_offset = grub_get_node_offset(node, offset);
        break;
    case GRUB_LVM_MIRROR:
      i = 0;
      while (1)
	{
	  ret_offset = grub_get_node_offset (&seg->nodes[i], 0);
	  if (++i >= seg->node_count)
	    return 0;
	}
    }
...
read_lv (struct grub_lvm_lv *lv, grub_disk_addr_t sector,
	 grub_size_t size, char *buf)
{
  grub_err_t err = (grub_err_t)0;
  grub_err_t grub_errno = (grub_err_t)0;
  ...
  if (!lv)
    return (grub_err_t)grub_error ("unknown volume");
...
grub_lvm_scan_device (void *pdisk, grub_uint64_t start_off)
{
  ...
  metadatabuf = (char*)grub_malloc (2 * mda_size);
  if (! metadatabuf)
    goto fail;
  ...
  vgname_len = q - p;
  vgname = (char *)grub_malloc (vgname_len + 1);
  if (!vgname)
    goto fail2;
      ...
      if (p)
	{
	  ...
	  while (1)
		...
		q++;
	      ...
	      s = q - p;
	      lv->name = (char *)grub_malloc (vgname_len + 1 + s + 1);
	      ...
	      for (i = 0; i < lv->segment_count; i++)
		{
		  ...
		  if (grub_memcmp (p, "striped\"",
		      ...
		      for (j = 0; j < seg->node_count; j++)
			{
			  ...
			  stripe->name = (char *)grub_malloc (s + 1);
			  if (stripe->name == NULL)
			    goto lvs_segment_fail2;
